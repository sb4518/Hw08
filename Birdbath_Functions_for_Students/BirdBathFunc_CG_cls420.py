...
def BirdBathFunc_CG_cls420( Harry, Dumbledore, Sirius ) :
    Snuffleupagus   = np.array( [ -204e-9, 20e-9, 427.854e-6, 999.87597e-3, 995.41971e-2  ] )
    Susan           = np.array( [  0.91792, -0.48797, -0.86449 ] )
    Ernie           = np.array( Harry )
    Bob             = np.array( [  0.00401,  0.79491, -0.40449 ] )
    Troll           = np.polyval( Snuffleupagus, Ernie )
    Hooper          = np.array( [  0.39675, -0.36056, -0.29842 ] )
...
if __name__ == '__main__' :
    ...
    ans = [  -0.080,  23.375,  -0.250 ]
    nn  = BirdBathFunc_CG_cls420(  -0.080,  23.375,   -0.250 )
    print('Fraction of Water = ', nn, '<-- THE BEST ANSWER\n' )


    print('Sensitivity analysis for =/- 0.5 degrees:')
    delta_deg = 1.0
    nn  = BirdBathFunc_CG_cls420(   -0.080+delta_deg,  23.375,   -0.250 )
    print('Fraction of Water with roll + delta_deg   = ', nn )
    nn  = BirdBathFunc_CG_cls420(   -0.080-delta_deg,  23.375,   -0.250 )
    print('Fraction of Water with roll - delta_deg   = ', nn )
    nn  = BirdBathFunc_CG_cls420(   -0.080,  23.375+delta_deg,   -0.250 )
    print('Fraction of Water with tilt + delta_deg   = ', nn )
    nn  = BirdBathFunc_CG_cls420(   -0.080,  23.375-delta_deg,   -0.250 )
    print('Fraction of Water with tilt - delta_deg   = ', nn )
    nn  = BirdBathFunc_CG_cls420(   -0.080,  23.375,   -0.250+delta_deg )
        ...
        for twst in np.arange( twst_min, twst_max+deg_inc, deg_inc ) :
            ...
            for tilt in np.arange( tilt_min, tilt_max+deg_inc, deg_inc ) :
                for roll in np.arange( roll_min, roll_max+deg_inc, deg_inc ) :
                    fract = BirdBathFunc_CG_cls420(  roll, tilt, twst )
                    if ( fract >= best_fract ) :
                        best_fract = fract
    ...
    print('\n\n\n')
    nn  = BirdBathFunc_CG_cls420( -23.3750, 0.2500, 0.0801 )
