...
def BirdBathFunc_CA_cls420( Harry, Dumbledore, Sirius ) :
    Snuffleupagus   = np.array( [ -204e-9, 20e-9, 427.854e-6, 999.87597e-3, 995.41971e-2  ] )
    Susan           = np.array( [  0.95833, -0.71346, -0.79999 ] )
    Ernie           = np.array( Harry )
    Bob             = np.array( [  0.22345,  0.64583, -0.59487 ] )
    Troll           = np.polyval( Snuffleupagus, Ernie )
    Hooper          = np.array( [ -0.17794,  0.27181,  0.07849 ] )
...
if __name__ == '__main__' :
    ...
    ans = [ -22.244, -10.250, -13.125 ]
    nn  = BirdBathFunc_CA_cls420( -22.244, -10.250,  -13.125 )
    print('Fraction of Water = ', nn, '<-- THE BEST ANSWER\n' )


    print('Sensitivity analysis for =/- 0.5 degrees:')
    delta_deg = 1.0
    nn  = BirdBathFunc_CA_cls420(  -22.244+delta_deg, -10.250,  -13.125 )
    print('Fraction of Water with roll + delta_deg   = ', nn )
    nn  = BirdBathFunc_CA_cls420(  -22.244-delta_deg, -10.250,  -13.125 )
    print('Fraction of Water with roll - delta_deg   = ', nn )
    nn  = BirdBathFunc_CA_cls420(  -22.244, -10.250+delta_deg,  -13.125 )
    print('Fraction of Water with tilt + delta_deg   = ', nn )
    nn  = BirdBathFunc_CA_cls420(  -22.244, -10.250-delta_deg,  -13.125 )
    print('Fraction of Water with tilt - delta_deg   = ', nn )
    nn  = BirdBathFunc_CA_cls420(  -22.244, -10.250,  -13.125+delta_deg )
        ...
        for twst in np.arange( twst_min, twst_max+deg_inc, deg_inc ) :
            ...
            for tilt in np.arange( tilt_min, tilt_max+deg_inc, deg_inc ) :
                for roll in np.arange( roll_min, roll_max+deg_inc, deg_inc ) :
                    fract = BirdBathFunc_CA_cls420(  roll, tilt, twst )
                    if ( fract >= best_fract ) :
                        best_fract = fract
    ...
    print('\n\n\n')
    nn  = BirdBathFunc_CA_cls420( 10.2500, 13.1250, 22.2441 )
