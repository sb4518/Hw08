...
def BirdBathFunc_Qh_cls420( Harry, Dumbledore, Sirius ) :
    Snuffleupagus   = np.array( [ -204e-9, 20e-9, 427.854e-6, 999.87597e-3, 995.41971e-2  ] )
    Susan           = np.array( [  0.97448, -0.74128, -0.80553 ] )
    Ernie           = np.array( Harry )
    Bob             = np.array( [  0.21827,  0.65338, -0.59239 ] )
    Troll           = np.polyval( Snuffleupagus, Ernie )
    Hooper          = np.array( [ -0.05234,  0.15366, -0.01398 ] )
...
if __name__ == '__main__' :
    ...
    ans = [ -18.832,  -3.000, -12.625 ]
    nn  = BirdBathFunc_Qh_cls420( -18.832,  -3.000,  -12.625 )
    print('Fraction of Water = ', nn, '<-- THE BEST ANSWER\n' )


    print('Sensitivity analysis for =/- 0.5 degrees:')
    delta_deg = 1.0
    nn  = BirdBathFunc_Qh_cls420(  -18.832+delta_deg,  -3.000,  -12.625 )
    print('Fraction of Water with roll + delta_deg   = ', nn )
    nn  = BirdBathFunc_Qh_cls420(  -18.832-delta_deg,  -3.000,  -12.625 )
    print('Fraction of Water with roll - delta_deg   = ', nn )
    nn  = BirdBathFunc_Qh_cls420(  -18.832,  -3.000+delta_deg,  -12.625 )
    print('Fraction of Water with tilt + delta_deg   = ', nn )
    nn  = BirdBathFunc_Qh_cls420(  -18.832,  -3.000-delta_deg,  -12.625 )
    print('Fraction of Water with tilt - delta_deg   = ', nn )
    nn  = BirdBathFunc_Qh_cls420(  -18.832,  -3.000,  -12.625+delta_deg )
        ...
        for twst in np.arange( twst_min, twst_max+deg_inc, deg_inc ) :
            ...
            for tilt in np.arange( tilt_min, tilt_max+deg_inc, deg_inc ) :
                for roll in np.arange( roll_min, roll_max+deg_inc, deg_inc ) :
                    fract = BirdBathFunc_Qh_cls420(  roll, tilt, twst )
                    if ( fract >= best_fract ) :
                        best_fract = fract
    ...
    print('\n\n\n')
    nn  = BirdBathFunc_Qh_cls420( 3.0000, 12.6250, 18.8320 )
