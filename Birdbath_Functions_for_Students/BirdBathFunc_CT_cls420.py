...
def BirdBathFunc_CT_cls420( Harry, Dumbledore, Sirius ) :
    Snuffleupagus   = np.array( [ -204e-9, 20e-9, 427.854e-6, 999.87597e-3, 995.41971e-2  ] )
    Susan           = np.array( [  0.94771, -0.47379, -0.90764 ] )
    Ernie           = np.array( Harry )
    Bob             = np.array( [ -0.04967,  0.82414, -0.35429 ] )
    Troll           = np.polyval( Snuffleupagus, Ernie )
    Hooper          = np.array( [  0.31523, -0.31036, -0.22510 ] )
...
if __name__ == '__main__' :
    ...
    ans = [  -0.580,  18.375,   3.000 ]
    nn  = BirdBathFunc_CT_cls420(  -0.580,  18.375,    3.000 )
    print('Fraction of Water = ', nn, '<-- THE BEST ANSWER\n' )


    print('Sensitivity analysis for =/- 0.5 degrees:')
    delta_deg = 1.0
    nn  = BirdBathFunc_CT_cls420(   -0.580+delta_deg,  18.375,    3.000 )
    print('Fraction of Water with roll + delta_deg   = ', nn )
    nn  = BirdBathFunc_CT_cls420(   -0.580-delta_deg,  18.375,    3.000 )
    print('Fraction of Water with roll - delta_deg   = ', nn )
    nn  = BirdBathFunc_CT_cls420(   -0.580,  18.375+delta_deg,    3.000 )
    print('Fraction of Water with tilt + delta_deg   = ', nn )
    nn  = BirdBathFunc_CT_cls420(   -0.580,  18.375-delta_deg,    3.000 )
    print('Fraction of Water with tilt - delta_deg   = ', nn )
    nn  = BirdBathFunc_CT_cls420(   -0.580,  18.375,    3.000+delta_deg )
        ...
        for twst in np.arange( twst_min, twst_max+deg_inc, deg_inc ) :
            ...
            for tilt in np.arange( tilt_min, tilt_max+deg_inc, deg_inc ) :
                for roll in np.arange( roll_min, roll_max+deg_inc, deg_inc ) :
                    fract = BirdBathFunc_CT_cls420(  roll, tilt, twst )
                    if ( fract >= best_fract ) :
                        best_fract = fract
    ...
    print('\n\n\n')
    nn  = BirdBathFunc_CT_cls420( -18.3750, -3.0000, 0.5801 )
