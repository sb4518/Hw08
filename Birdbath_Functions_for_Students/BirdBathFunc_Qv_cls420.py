...
def BirdBathFunc_Qv_cls420( Harry, Dumbledore, Sirius ) :
    Snuffleupagus   = np.array( [ -204e-9, 20e-9, 427.854e-6, 999.87597e-3, 995.41971e-2  ] )
    Susan           = np.array( [  0.96869, -0.59170, -0.87373 ] )
    Ernie           = np.array( Harry )
    Bob             = np.array( [  0.00211,  0.80348, -0.40651 ] )
    Troll           = np.polyval( Snuffleupagus, Ernie )
    Hooper          = np.array( [  0.24827, -0.06571, -0.26713 ] )
...
if __name__ == '__main__' :
    ...
    ans = [ -15.926,  14.375,  -0.125 ]
    nn  = BirdBathFunc_Qv_cls420( -15.926,  14.375,   -0.125 )
    print('Fraction of Water = ', nn, '<-- THE BEST ANSWER\n' )


    print('Sensitivity analysis for =/- 0.5 degrees:')
    delta_deg = 1.0
    nn  = BirdBathFunc_Qv_cls420(  -15.926+delta_deg,  14.375,   -0.125 )
    print('Fraction of Water with roll + delta_deg   = ', nn )
    nn  = BirdBathFunc_Qv_cls420(  -15.926-delta_deg,  14.375,   -0.125 )
    print('Fraction of Water with roll - delta_deg   = ', nn )
    nn  = BirdBathFunc_Qv_cls420(  -15.926,  14.375+delta_deg,   -0.125 )
    print('Fraction of Water with tilt + delta_deg   = ', nn )
    nn  = BirdBathFunc_Qv_cls420(  -15.926,  14.375-delta_deg,   -0.125 )
    print('Fraction of Water with tilt - delta_deg   = ', nn )
    nn  = BirdBathFunc_Qv_cls420(  -15.926,  14.375,   -0.125+delta_deg )
        ...
        for twst in np.arange( twst_min, twst_max+deg_inc, deg_inc ) :
            ...
            for tilt in np.arange( tilt_min, tilt_max+deg_inc, deg_inc ) :
                for roll in np.arange( roll_min, roll_max+deg_inc, deg_inc ) :
                    fract = BirdBathFunc_Qv_cls420(  roll, tilt, twst )
                    if ( fract >= best_fract ) :
                        best_fract = fract
    ...
    print('\n\n\n')
    nn  = BirdBathFunc_Qv_cls420( -14.3750, 0.1250, 15.9258 )
