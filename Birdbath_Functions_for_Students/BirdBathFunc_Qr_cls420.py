...
def BirdBathFunc_Qr_cls420( Harry, Dumbledore, Sirius ) :
    Snuffleupagus   = np.array( [ -204e-9, 20e-9, 427.854e-6, 999.87597e-3, 995.41971e-2  ] )
    Susan           = np.array( [  0.99979, -0.59749, -0.90841 ] )
    Ernie           = np.array( Harry )
    Bob             = np.array( [  0.00654,  0.76768, -0.39387 ] )
    Troll           = np.polyval( Snuffleupagus, Ernie )
    Hooper          = np.array( [  0.01963,  0.23169, -0.14022 ] )
...
if __name__ == '__main__' :
    ...
    ans = [ -27.666,   1.125,  -0.375 ]
    nn  = BirdBathFunc_Qr_cls420( -27.666,   1.125,   -0.375 )
    print('Fraction of Water = ', nn, '<-- THE BEST ANSWER\n' )


    print('Sensitivity analysis for =/- 0.5 degrees:')
    delta_deg = 1.0
    nn  = BirdBathFunc_Qr_cls420(  -27.666+delta_deg,   1.125,   -0.375 )
    print('Fraction of Water with roll + delta_deg   = ', nn )
    nn  = BirdBathFunc_Qr_cls420(  -27.666-delta_deg,   1.125,   -0.375 )
    print('Fraction of Water with roll - delta_deg   = ', nn )
    nn  = BirdBathFunc_Qr_cls420(  -27.666,   1.125+delta_deg,   -0.375 )
    print('Fraction of Water with tilt + delta_deg   = ', nn )
    nn  = BirdBathFunc_Qr_cls420(  -27.666,   1.125-delta_deg,   -0.375 )
    print('Fraction of Water with tilt - delta_deg   = ', nn )
    nn  = BirdBathFunc_Qr_cls420(  -27.666,   1.125,   -0.375+delta_deg )
        ...
        for twst in np.arange( twst_min, twst_max+deg_inc, deg_inc ) :
            ...
            for tilt in np.arange( tilt_min, tilt_max+deg_inc, deg_inc ) :
                for roll in np.arange( roll_min, roll_max+deg_inc, deg_inc ) :
                    fract = BirdBathFunc_Qr_cls420(  roll, tilt, twst )
                    if ( fract >= best_fract ) :
                        best_fract = fract
    ...
    print('\n\n\n')
    nn  = BirdBathFunc_Qr_cls420( -1.1250, 0.3750, 27.6660 )
