...
def BirdBathFunc_Qq_cls420( Harry, Dumbledore, Sirius ) :
    Snuffleupagus   = np.array( [ -204e-9, 20e-9, 427.854e-6, 999.87597e-3, 995.41971e-2  ] )
    Susan           = np.array( [  0.97167, -0.75734, -0.79406 ] )
    Ernie           = np.array( Harry )
    Bob             = np.array( [  0.23552,  0.63973, -0.60639 ] )
    Troll           = np.polyval( Snuffleupagus, Ernie )
    Hooper          = np.array( [ -0.01963,  0.13110, -0.04217 ] )
...
if __name__ == '__main__' :
    ...
    ans = [ -18.580,  -1.125, -13.625 ]
    nn  = BirdBathFunc_Qq_cls420( -18.580,  -1.125,  -13.625 )
    print('Fraction of Water = ', nn, '<-- THE BEST ANSWER\n' )


    print('Sensitivity analysis for =/- 0.5 degrees:')
    delta_deg = 1.0
    nn  = BirdBathFunc_Qq_cls420(  -18.580+delta_deg,  -1.125,  -13.625 )
    print('Fraction of Water with roll + delta_deg   = ', nn )
    nn  = BirdBathFunc_Qq_cls420(  -18.580-delta_deg,  -1.125,  -13.625 )
    print('Fraction of Water with roll - delta_deg   = ', nn )
    nn  = BirdBathFunc_Qq_cls420(  -18.580,  -1.125+delta_deg,  -13.625 )
    print('Fraction of Water with tilt + delta_deg   = ', nn )
    nn  = BirdBathFunc_Qq_cls420(  -18.580,  -1.125-delta_deg,  -13.625 )
    print('Fraction of Water with tilt - delta_deg   = ', nn )
    nn  = BirdBathFunc_Qq_cls420(  -18.580,  -1.125,  -13.625+delta_deg )
        ...
        for twst in np.arange( twst_min, twst_max+deg_inc, deg_inc ) :
            ...
            for tilt in np.arange( tilt_min, tilt_max+deg_inc, deg_inc ) :
                for roll in np.arange( roll_min, roll_max+deg_inc, deg_inc ) :
                    fract = BirdBathFunc_Qq_cls420(  roll, tilt, twst )
                    if ( fract >= best_fract ) :
                        best_fract = fract
    ...
    print('\n\n\n')
    nn  = BirdBathFunc_Qq_cls420( 1.1250, 13.6250, 18.5801 )
