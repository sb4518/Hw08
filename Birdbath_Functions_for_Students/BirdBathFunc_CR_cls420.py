...
def BirdBathFunc_CR_cls420( Harry, Dumbledore, Sirius ) :
    Snuffleupagus   = np.array( [ -204e-9, 20e-9, 427.854e-6, 999.87597e-3, 995.41971e-2  ] )
    Susan           = np.array( [  0.99161, -0.59509, -0.89974 ] )
    Ernie           = np.array( Harry )
    Bob             = np.array( [ -0.01514,  0.79585, -0.38181 ] )
    Troll           = np.polyval( Snuffleupagus, Ernie )
    Hooper          = np.array( [ -0.12836, -0.11185,  0.21143 ] )
...
if __name__ == '__main__' :
    ...
    ans = [   3.541,  -7.375,   0.875 ]
    nn  = BirdBathFunc_CR_cls420(   3.541,  -7.375,    0.875 )
    print('Fraction of Water = ', nn, '<-- THE BEST ANSWER\n' )


    print('Sensitivity analysis for =/- 0.5 degrees:')
    delta_deg = 1.0
    nn  = BirdBathFunc_CR_cls420(    3.541+delta_deg,  -7.375,    0.875 )
    print('Fraction of Water with roll + delta_deg   = ', nn )
    nn  = BirdBathFunc_CR_cls420(    3.541-delta_deg,  -7.375,    0.875 )
    print('Fraction of Water with roll - delta_deg   = ', nn )
    nn  = BirdBathFunc_CR_cls420(    3.541,  -7.375+delta_deg,    0.875 )
    print('Fraction of Water with tilt + delta_deg   = ', nn )
    nn  = BirdBathFunc_CR_cls420(    3.541,  -7.375-delta_deg,    0.875 )
    print('Fraction of Water with tilt - delta_deg   = ', nn )
    nn  = BirdBathFunc_CR_cls420(    3.541,  -7.375,    0.875+delta_deg )
        ...
        for twst in np.arange( twst_min, twst_max+deg_inc, deg_inc ) :
            ...
            for tilt in np.arange( tilt_min, tilt_max+deg_inc, deg_inc ) :
                for roll in np.arange( roll_min, roll_max+deg_inc, deg_inc ) :
                    fract = BirdBathFunc_CR_cls420(  roll, tilt, twst )
                    if ( fract >= best_fract ) :
                        best_fract = fract
    ...
    print('\n\n\n')
    nn  = BirdBathFunc_CR_cls420( 7.3750, -0.8750, -3.5410 )
