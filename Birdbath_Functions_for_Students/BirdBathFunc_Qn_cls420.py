...
def BirdBathFunc_Qn_cls420( Harry, Dumbledore, Sirius ) :
    Snuffleupagus   = np.array( [ -204e-9, 20e-9, 427.854e-6, 999.87597e-3, 995.41971e-2  ] )
    Susan           = np.array( [  0.98731, -0.70651, -0.83852 ] )
    Ernie           = np.array( Harry )
    Bob             = np.array( [  0.15858,  0.68157, -0.53440 ] )
    Troll           = np.polyval( Snuffleupagus, Ernie )
    Hooper          = np.array( [ -0.00873, -0.19058,  0.10637 ] )
...
if __name__ == '__main__' :
    ...
    ans = [   4.039,  -0.500,  -9.125 ]
    nn  = BirdBathFunc_Qn_cls420(   4.039,  -0.500,   -9.125 )
    print('Fraction of Water = ', nn, '<-- THE BEST ANSWER\n' )


    print('Sensitivity analysis for =/- 0.5 degrees:')
    delta_deg = 1.0
    nn  = BirdBathFunc_Qn_cls420(    4.039+delta_deg,  -0.500,   -9.125 )
    print('Fraction of Water with roll + delta_deg   = ', nn )
    nn  = BirdBathFunc_Qn_cls420(    4.039-delta_deg,  -0.500,   -9.125 )
    print('Fraction of Water with roll - delta_deg   = ', nn )
    nn  = BirdBathFunc_Qn_cls420(    4.039,  -0.500+delta_deg,   -9.125 )
    print('Fraction of Water with tilt + delta_deg   = ', nn )
    nn  = BirdBathFunc_Qn_cls420(    4.039,  -0.500-delta_deg,   -9.125 )
    print('Fraction of Water with tilt - delta_deg   = ', nn )
    nn  = BirdBathFunc_Qn_cls420(    4.039,  -0.500,   -9.125+delta_deg )
        ...
        for twst in np.arange( twst_min, twst_max+deg_inc, deg_inc ) :
            ...
            for tilt in np.arange( tilt_min, tilt_max+deg_inc, deg_inc ) :
                for roll in np.arange( roll_min, roll_max+deg_inc, deg_inc ) :
                    fract = BirdBathFunc_Qn_cls420(  roll, tilt, twst )
                    if ( fract >= best_fract ) :
                        best_fract = fract
    ...
    print('\n\n\n')
    nn  = BirdBathFunc_Qn_cls420( 0.5000, 9.1250, -4.0391 )
