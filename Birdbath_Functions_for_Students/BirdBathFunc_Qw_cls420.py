...
def BirdBathFunc_Qw_cls420( Harry, Dumbledore, Sirius ) :
    Snuffleupagus   = np.array( [ -204e-9, 20e-9, 427.854e-6, 999.87597e-3, 995.41971e-2  ] )
    Susan           = np.array( [  0.98711, -0.52841, -0.92782 ] )
    Ernie           = np.array( Harry )
    Bob             = np.array( [ -0.02154,  0.79036, -0.37132 ] )
    Troll           = np.polyval( Snuffleupagus, Ernie )
    Hooper          = np.array( [ -0.15859,  0.31004,  0.03587 ] )
...
if __name__ == '__main__' :
    ...
    ans = [ -25.902,  -9.125,   1.250 ]
    nn  = BirdBathFunc_Qw_cls420( -25.902,  -9.125,    1.250 )
    print('Fraction of Water = ', nn, '<-- THE BEST ANSWER\n' )


    print('Sensitivity analysis for =/- 0.5 degrees:')
    delta_deg = 1.0
    nn  = BirdBathFunc_Qw_cls420(  -25.902+delta_deg,  -9.125,    1.250 )
    print('Fraction of Water with roll + delta_deg   = ', nn )
    nn  = BirdBathFunc_Qw_cls420(  -25.902-delta_deg,  -9.125,    1.250 )
    print('Fraction of Water with roll - delta_deg   = ', nn )
    nn  = BirdBathFunc_Qw_cls420(  -25.902,  -9.125+delta_deg,    1.250 )
    print('Fraction of Water with tilt + delta_deg   = ', nn )
    nn  = BirdBathFunc_Qw_cls420(  -25.902,  -9.125-delta_deg,    1.250 )
    print('Fraction of Water with tilt - delta_deg   = ', nn )
    nn  = BirdBathFunc_Qw_cls420(  -25.902,  -9.125,    1.250+delta_deg )
        ...
        for twst in np.arange( twst_min, twst_max+deg_inc, deg_inc ) :
            ...
            for tilt in np.arange( tilt_min, tilt_max+deg_inc, deg_inc ) :
                for roll in np.arange( roll_min, roll_max+deg_inc, deg_inc ) :
                    fract = BirdBathFunc_Qw_cls420(  roll, tilt, twst )
                    if ( fract >= best_fract ) :
                        best_fract = fract
    ...
    print('\n\n\n')
    nn  = BirdBathFunc_Qw_cls420( 9.1250, -1.2500, 25.9023 )
