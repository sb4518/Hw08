...
def BirdBathFunc_CV_cls420( Harry, Dumbledore, Sirius ) :
    Snuffleupagus   = np.array( [ -204e-9, 20e-9, 427.854e-6, 999.87597e-3, 995.41971e-2  ] )
    Susan           = np.array( [  0.98330, -0.63175, -0.87126 ] )
    Ernie           = np.array( Harry )
    Bob             = np.array( [  0.06660,  0.76806, -0.46668 ] )
    Troll           = np.polyval( Snuffleupagus, Ernie )
    Hooper          = np.array( [ -0.16935,  0.10476,  0.15209 ] )
...
if __name__ == '__main__' :
    ...
    ans = [ -10.375,  -9.750,  -3.875 ]
    nn  = BirdBathFunc_CV_cls420( -10.375,  -9.750,   -3.875 )
    print('Fraction of Water = ', nn, '<-- THE BEST ANSWER\n' )


    print('Sensitivity analysis for =/- 0.5 degrees:')
    delta_deg = 1.0
    nn  = BirdBathFunc_CV_cls420(  -10.375+delta_deg,  -9.750,   -3.875 )
    print('Fraction of Water with roll + delta_deg   = ', nn )
    nn  = BirdBathFunc_CV_cls420(  -10.375-delta_deg,  -9.750,   -3.875 )
    print('Fraction of Water with roll - delta_deg   = ', nn )
    nn  = BirdBathFunc_CV_cls420(  -10.375,  -9.750+delta_deg,   -3.875 )
    print('Fraction of Water with tilt + delta_deg   = ', nn )
    nn  = BirdBathFunc_CV_cls420(  -10.375,  -9.750-delta_deg,   -3.875 )
    print('Fraction of Water with tilt - delta_deg   = ', nn )
    nn  = BirdBathFunc_CV_cls420(  -10.375,  -9.750,   -3.875+delta_deg )
        ...
        for twst in np.arange( twst_min, twst_max+deg_inc, deg_inc ) :
            ...
            for tilt in np.arange( tilt_min, tilt_max+deg_inc, deg_inc ) :
                for roll in np.arange( roll_min, roll_max+deg_inc, deg_inc ) :
                    fract = BirdBathFunc_CV_cls420(  roll, tilt, twst )
                    if ( fract >= best_fract ) :
                        best_fract = fract
    ...
    print('\n\n\n')
    nn  = BirdBathFunc_CV_cls420( 9.7500, 3.8750, 10.3750 )
