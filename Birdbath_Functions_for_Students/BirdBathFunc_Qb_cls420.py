...
def BirdBathFunc_Qb_cls420( Harry, Dumbledore, Sirius ) :
    Snuffleupagus   = np.array( [ -204e-9, 20e-9, 427.854e-6, 999.87597e-3, 995.41971e-2  ] )
    Susan           = np.array( [  0.96404, -0.55682, -0.88564 ] )
    Ernie           = np.array( Harry )
    Bob             = np.array( [ -0.03788,  0.82727, -0.37012 ] )
    Troll           = np.polyval( Snuffleupagus, Ernie )
    Hooper          = np.array( [  0.26303, -0.07471, -0.28046 ] )
...
if __name__ == '__main__' :
    ...
    ans = [ -15.926,  15.250,   2.250 ]
    nn  = BirdBathFunc_Qb_cls420( -15.926,  15.250,    2.250 )
    print('Fraction of Water = ', nn, '<-- THE BEST ANSWER\n' )


    print('Sensitivity analysis for =/- 0.5 degrees:')
    delta_deg = 1.0
    nn  = BirdBathFunc_Qb_cls420(  -15.926+delta_deg,  15.250,    2.250 )
    print('Fraction of Water with roll + delta_deg   = ', nn )
    nn  = BirdBathFunc_Qb_cls420(  -15.926-delta_deg,  15.250,    2.250 )
    print('Fraction of Water with roll - delta_deg   = ', nn )
    nn  = BirdBathFunc_Qb_cls420(  -15.926,  15.250+delta_deg,    2.250 )
    print('Fraction of Water with tilt + delta_deg   = ', nn )
    nn  = BirdBathFunc_Qb_cls420(  -15.926,  15.250-delta_deg,    2.250 )
    print('Fraction of Water with tilt - delta_deg   = ', nn )
    nn  = BirdBathFunc_Qb_cls420(  -15.926,  15.250,    2.250+delta_deg )
        ...
        for twst in np.arange( twst_min, twst_max+deg_inc, deg_inc ) :
            ...
            for tilt in np.arange( tilt_min, tilt_max+deg_inc, deg_inc ) :
                for roll in np.arange( roll_min, roll_max+deg_inc, deg_inc ) :
                    fract = BirdBathFunc_Qb_cls420(  roll, tilt, twst )
                    if ( fract >= best_fract ) :
                        best_fract = fract
    ...
    print('\n\n\n')
    nn  = BirdBathFunc_Qb_cls420( -15.2500, -2.2500, 15.9258 )
