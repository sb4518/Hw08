...
def BirdBathFunc_CE_cls420( Harry, Dumbledore, Sirius ) :
    Snuffleupagus   = np.array( [ -204e-9, 20e-9, 427.854e-6, 999.87597e-3, 995.41971e-2  ] )
    Susan           = np.array( [  0.97137, -0.73303, -0.80591 ] )
    Ernie           = np.array( Harry )
    Bob             = np.array( [  0.22426,  0.63727, -0.59153 ] )
    Troll           = np.polyval( Snuffleupagus, Ernie )
    Hooper          = np.array( [ -0.07846,  0.23782, -0.02470 ] )
...
if __name__ == '__main__' :
    ...
    ans = [ -23.885,  -4.500, -13.000 ]
    nn  = BirdBathFunc_CE_cls420( -23.885,  -4.500,  -13.000 )
    print('Fraction of Water = ', nn, '<-- THE BEST ANSWER\n' )


    print('Sensitivity analysis for =/- 0.5 degrees:')
    delta_deg = 1.0
    nn  = BirdBathFunc_CE_cls420(  -23.885+delta_deg,  -4.500,  -13.000 )
    print('Fraction of Water with roll + delta_deg   = ', nn )
    nn  = BirdBathFunc_CE_cls420(  -23.885-delta_deg,  -4.500,  -13.000 )
    print('Fraction of Water with roll - delta_deg   = ', nn )
    nn  = BirdBathFunc_CE_cls420(  -23.885,  -4.500+delta_deg,  -13.000 )
    print('Fraction of Water with tilt + delta_deg   = ', nn )
    nn  = BirdBathFunc_CE_cls420(  -23.885,  -4.500-delta_deg,  -13.000 )
    print('Fraction of Water with tilt - delta_deg   = ', nn )
    nn  = BirdBathFunc_CE_cls420(  -23.885,  -4.500,  -13.000+delta_deg )
        ...
        for twst in np.arange( twst_min, twst_max+deg_inc, deg_inc ) :
            ...
            for tilt in np.arange( tilt_min, tilt_max+deg_inc, deg_inc ) :
                for roll in np.arange( roll_min, roll_max+deg_inc, deg_inc ) :
                    fract = BirdBathFunc_CE_cls420(  roll, tilt, twst )
                    if ( fract >= best_fract ) :
                        best_fract = fract
    ...
    print('\n\n\n')
    nn  = BirdBathFunc_CE_cls420( 4.5000, 13.0000, 23.8848 )
