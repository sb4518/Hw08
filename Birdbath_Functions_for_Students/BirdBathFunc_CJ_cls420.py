...
def BirdBathFunc_CJ_cls420( Harry, Dumbledore, Sirius ) :
    Snuffleupagus   = np.array( [ -204e-9, 20e-9, 427.854e-6, 999.87597e-3, 995.41971e-2  ] )
    Susan           = np.array( [  0.91849, -0.60278, -0.80747 ] )
    Ernie           = np.array( Harry )
    Bob             = np.array( [ -0.04613,  0.79789, -0.34537 ] )
    Troll           = np.polyval( Snuffleupagus, Ernie )
    Hooper          = np.array( [  0.39274,  0.00677, -0.47825 ] )
...
if __name__ == '__main__' :
    ...
    ans = [ -28.797,  23.125,   2.875 ]
    nn  = BirdBathFunc_CJ_cls420( -28.797,  23.125,    2.875 )
    print('Fraction of Water = ', nn, '<-- THE BEST ANSWER\n' )


    print('Sensitivity analysis for =/- 0.5 degrees:')
    delta_deg = 1.0
    nn  = BirdBathFunc_CJ_cls420(  -28.797+delta_deg,  23.125,    2.875 )
    print('Fraction of Water with roll + delta_deg   = ', nn )
    nn  = BirdBathFunc_CJ_cls420(  -28.797-delta_deg,  23.125,    2.875 )
    print('Fraction of Water with roll - delta_deg   = ', nn )
    nn  = BirdBathFunc_CJ_cls420(  -28.797,  23.125+delta_deg,    2.875 )
    print('Fraction of Water with tilt + delta_deg   = ', nn )
    nn  = BirdBathFunc_CJ_cls420(  -28.797,  23.125-delta_deg,    2.875 )
    print('Fraction of Water with tilt - delta_deg   = ', nn )
    nn  = BirdBathFunc_CJ_cls420(  -28.797,  23.125,    2.875+delta_deg )
        ...
        for twst in np.arange( twst_min, twst_max+deg_inc, deg_inc ) :
            ...
            for tilt in np.arange( tilt_min, tilt_max+deg_inc, deg_inc ) :
                for roll in np.arange( roll_min, roll_max+deg_inc, deg_inc ) :
                    fract = BirdBathFunc_CJ_cls420(  roll, tilt, twst )
                    if ( fract >= best_fract ) :
                        best_fract = fract
    ...
    print('\n\n\n')
    nn  = BirdBathFunc_CJ_cls420( -23.1250, -2.8750, 28.7969 )
