...
def BirdBathFunc_CX_cls420( Harry, Dumbledore, Sirius ) :
    Snuffleupagus   = np.array( [ -204e-9, 20e-9, 427.854e-6, 999.87597e-3, 995.41971e-2  ] )
    Susan           = np.array( [  0.92006, -0.56599, -0.82786 ] )
    Ernie           = np.array( Harry )
    Bob             = np.array( [ -0.09265,  0.82406, -0.30228 ] )
    Troll           = np.polyval( Snuffleupagus, Ernie )
    Hooper          = np.array( [  0.38067,  0.02443, -0.47254 ] )
...
if __name__ == '__main__' :
    ...
    ans = [ -29.551,  22.375,   5.750 ]
    nn  = BirdBathFunc_CX_cls420( -29.551,  22.375,    5.750 )
    print('Fraction of Water = ', nn, '<-- THE BEST ANSWER\n' )


    print('Sensitivity analysis for =/- 0.5 degrees:')
    delta_deg = 1.0
    nn  = BirdBathFunc_CX_cls420(  -29.551+delta_deg,  22.375,    5.750 )
    print('Fraction of Water with roll + delta_deg   = ', nn )
    nn  = BirdBathFunc_CX_cls420(  -29.551-delta_deg,  22.375,    5.750 )
    print('Fraction of Water with roll - delta_deg   = ', nn )
    nn  = BirdBathFunc_CX_cls420(  -29.551,  22.375+delta_deg,    5.750 )
    print('Fraction of Water with tilt + delta_deg   = ', nn )
    nn  = BirdBathFunc_CX_cls420(  -29.551,  22.375-delta_deg,    5.750 )
    print('Fraction of Water with tilt - delta_deg   = ', nn )
    nn  = BirdBathFunc_CX_cls420(  -29.551,  22.375,    5.750+delta_deg )
        ...
        for twst in np.arange( twst_min, twst_max+deg_inc, deg_inc ) :
            ...
            for tilt in np.arange( tilt_min, tilt_max+deg_inc, deg_inc ) :
                for roll in np.arange( roll_min, roll_max+deg_inc, deg_inc ) :
                    fract = BirdBathFunc_CX_cls420(  roll, tilt, twst )
                    if ( fract >= best_fract ) :
                        best_fract = fract
    ...
    print('\n\n\n')
    nn  = BirdBathFunc_CX_cls420( -22.3750, -5.7500, 29.5508 )
