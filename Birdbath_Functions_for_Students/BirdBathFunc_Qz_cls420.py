...
def BirdBathFunc_Qz_cls420( Harry, Dumbledore, Sirius ) :
    Snuffleupagus   = np.array( [ -204e-9, 20e-9, 427.854e-6, 999.87597e-3, 995.41971e-2  ] )
    Susan           = np.array( [  0.98596, -0.62922, -0.87575 ] )
    Ernie           = np.array( Harry )
    Bob             = np.array( [  0.03658,  0.77681, -0.43478 ] )
    Troll           = np.polyval( Snuffleupagus, Ernie )
    Hooper          = np.array( [ -0.16290, -0.02568,  0.20986 ] )
...
if __name__ == '__main__' :
    ...
    ans = [  -1.205,  -9.375,  -2.125 ]
    nn  = BirdBathFunc_Qz_cls420(  -1.205,  -9.375,   -2.125 )
    print('Fraction of Water = ', nn, '<-- THE BEST ANSWER\n' )


    print('Sensitivity analysis for =/- 0.5 degrees:')
    delta_deg = 1.0
    nn  = BirdBathFunc_Qz_cls420(   -1.205+delta_deg,  -9.375,   -2.125 )
    print('Fraction of Water with roll + delta_deg   = ', nn )
    nn  = BirdBathFunc_Qz_cls420(   -1.205-delta_deg,  -9.375,   -2.125 )
    print('Fraction of Water with roll - delta_deg   = ', nn )
    nn  = BirdBathFunc_Qz_cls420(   -1.205,  -9.375+delta_deg,   -2.125 )
    print('Fraction of Water with tilt + delta_deg   = ', nn )
    nn  = BirdBathFunc_Qz_cls420(   -1.205,  -9.375-delta_deg,   -2.125 )
    print('Fraction of Water with tilt - delta_deg   = ', nn )
    nn  = BirdBathFunc_Qz_cls420(   -1.205,  -9.375,   -2.125+delta_deg )
        ...
        for twst in np.arange( twst_min, twst_max+deg_inc, deg_inc ) :
            ...
            for tilt in np.arange( tilt_min, tilt_max+deg_inc, deg_inc ) :
                for roll in np.arange( roll_min, roll_max+deg_inc, deg_inc ) :
                    fract = BirdBathFunc_Qz_cls420(  roll, tilt, twst )
                    if ( fract >= best_fract ) :
                        best_fract = fract
    ...
    print('\n\n\n')
    nn  = BirdBathFunc_Qz_cls420( 9.3750, 2.1250, 1.2051 )
