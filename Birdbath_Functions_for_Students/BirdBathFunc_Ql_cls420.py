...
def BirdBathFunc_Ql_cls420( Harry, Dumbledore, Sirius ) :
    Snuffleupagus   = np.array( [ -204e-9, 20e-9, 427.854e-6, 999.87597e-3, 995.41971e-2  ] )
    Susan           = np.array( [  0.99199, -0.60403, -0.89571 ] )
    Ernie           = np.array( Harry )
    Bob             = np.array( [  0.02381,  0.79466, -0.42831 ] )
    Troll           = np.polyval( Snuffleupagus, Ernie )
    Hooper          = np.array( [ -0.12403,  0.06065,  0.11948 ] )
...
if __name__ == '__main__' :
    ...
    ans = [  -9.115,  -7.125,  -1.375 ]
    nn  = BirdBathFunc_Ql_cls420(  -9.115,  -7.125,   -1.375 )
    print('Fraction of Water = ', nn, '<-- THE BEST ANSWER\n' )


    print('Sensitivity analysis for =/- 0.5 degrees:')
    delta_deg = 1.0
    nn  = BirdBathFunc_Ql_cls420(   -9.115+delta_deg,  -7.125,   -1.375 )
    print('Fraction of Water with roll + delta_deg   = ', nn )
    nn  = BirdBathFunc_Ql_cls420(   -9.115-delta_deg,  -7.125,   -1.375 )
    print('Fraction of Water with roll - delta_deg   = ', nn )
    nn  = BirdBathFunc_Ql_cls420(   -9.115,  -7.125+delta_deg,   -1.375 )
    print('Fraction of Water with tilt + delta_deg   = ', nn )
    nn  = BirdBathFunc_Ql_cls420(   -9.115,  -7.125-delta_deg,   -1.375 )
    print('Fraction of Water with tilt - delta_deg   = ', nn )
    nn  = BirdBathFunc_Ql_cls420(   -9.115,  -7.125,   -1.375+delta_deg )
        ...
        for twst in np.arange( twst_min, twst_max+deg_inc, deg_inc ) :
            ...
            for tilt in np.arange( tilt_min, tilt_max+deg_inc, deg_inc ) :
                for roll in np.arange( roll_min, roll_max+deg_inc, deg_inc ) :
                    fract = BirdBathFunc_Ql_cls420(  roll, tilt, twst )
                    if ( fract >= best_fract ) :
                        best_fract = fract
    ...
    print('\n\n\n')
    nn  = BirdBathFunc_Ql_cls420( 7.1250, 1.3750, 9.1152 )
