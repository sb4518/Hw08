...
def BirdBathFunc_Qs_cls420( Harry, Dumbledore, Sirius ) :
    Snuffleupagus   = np.array( [ -204e-9, 20e-9, 427.854e-6, 999.87597e-3, 995.41971e-2  ] )
    Susan           = np.array( [  0.96388, -0.59614, -0.86568 ] )
    Ernie           = np.array( Harry )
    Bob             = np.array( [  0.02524,  0.79306, -0.42923 ] )
    Troll           = np.polyval( Snuffleupagus, Ernie )
    Hooper          = np.array( [  0.26514, -0.12522, -0.25761 ] )
...
if __name__ == '__main__' :
    ...
    ans = [ -12.266,  15.375,  -1.500 ]
    nn  = BirdBathFunc_Qs_cls420( -12.266,  15.375,   -1.500 )
    print('Fraction of Water = ', nn, '<-- THE BEST ANSWER\n' )


    print('Sensitivity analysis for =/- 0.5 degrees:')
    delta_deg = 1.0
    nn  = BirdBathFunc_Qs_cls420(  -12.266+delta_deg,  15.375,   -1.500 )
    print('Fraction of Water with roll + delta_deg   = ', nn )
    nn  = BirdBathFunc_Qs_cls420(  -12.266-delta_deg,  15.375,   -1.500 )
    print('Fraction of Water with roll - delta_deg   = ', nn )
    nn  = BirdBathFunc_Qs_cls420(  -12.266,  15.375+delta_deg,   -1.500 )
    print('Fraction of Water with tilt + delta_deg   = ', nn )
    nn  = BirdBathFunc_Qs_cls420(  -12.266,  15.375-delta_deg,   -1.500 )
    print('Fraction of Water with tilt - delta_deg   = ', nn )
    nn  = BirdBathFunc_Qs_cls420(  -12.266,  15.375,   -1.500+delta_deg )
        ...
        for twst in np.arange( twst_min, twst_max+deg_inc, deg_inc ) :
            ...
            for tilt in np.arange( tilt_min, tilt_max+deg_inc, deg_inc ) :
                for roll in np.arange( roll_min, roll_max+deg_inc, deg_inc ) :
                    fract = BirdBathFunc_Qs_cls420(  roll, tilt, twst )
                    if ( fract >= best_fract ) :
                        best_fract = fract
    ...
    print('\n\n\n')
    nn  = BirdBathFunc_Qs_cls420( -15.3750, 1.5000, 12.2656 )
