...
def BirdBathFunc_CI_cls420( Harry, Dumbledore, Sirius ) :
    Snuffleupagus   = np.array( [ -204e-9, 20e-9, 427.854e-6, 999.87597e-3, 995.41971e-2  ] )
    Susan           = np.array( [  0.97934, -0.71843, -0.82289 ] )
    Ernie           = np.array( Harry )
    Bob             = np.array( [  0.20147,  0.60454, -0.54753 ] )
    Troll           = np.polyval( Snuffleupagus, Ernie )
    Hooper          = np.array( [ -0.01745,  0.34410, -0.15190 ] )
...
if __name__ == '__main__' :
    ...
    ans = [ -34.553,  -1.000, -11.625 ]
    nn  = BirdBathFunc_CI_cls420( -34.553,  -1.000,  -11.625 )
    print('Fraction of Water = ', nn, '<-- THE BEST ANSWER\n' )


    print('Sensitivity analysis for =/- 0.5 degrees:')
    delta_deg = 1.0
    nn  = BirdBathFunc_CI_cls420(  -34.553+delta_deg,  -1.000,  -11.625 )
    print('Fraction of Water with roll + delta_deg   = ', nn )
    nn  = BirdBathFunc_CI_cls420(  -34.553-delta_deg,  -1.000,  -11.625 )
    print('Fraction of Water with roll - delta_deg   = ', nn )
    nn  = BirdBathFunc_CI_cls420(  -34.553,  -1.000+delta_deg,  -11.625 )
    print('Fraction of Water with tilt + delta_deg   = ', nn )
    nn  = BirdBathFunc_CI_cls420(  -34.553,  -1.000-delta_deg,  -11.625 )
    print('Fraction of Water with tilt - delta_deg   = ', nn )
    nn  = BirdBathFunc_CI_cls420(  -34.553,  -1.000,  -11.625+delta_deg )
        ...
        for twst in np.arange( twst_min, twst_max+deg_inc, deg_inc ) :
            ...
            for tilt in np.arange( tilt_min, tilt_max+deg_inc, deg_inc ) :
                for roll in np.arange( roll_min, roll_max+deg_inc, deg_inc ) :
                    fract = BirdBathFunc_CI_cls420(  roll, tilt, twst )
                    if ( fract >= best_fract ) :
                        best_fract = fract
    ...
    print('\n\n\n')
    nn  = BirdBathFunc_CI_cls420( 1.0000, 11.6250, 34.5527 )
