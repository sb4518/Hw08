...
def BirdBathFunc_CD_cls420( Harry, Dumbledore, Sirius ) :
    Snuffleupagus   = np.array( [ -204e-9, 20e-9, 427.854e-6, 999.87597e-3, 995.41971e-2  ] )
    Susan           = np.array( [  0.98832, -0.59121, -0.89772 ] )
    Ernie           = np.array( Harry )
    Bob             = np.array( [  0.00862,  0.80361, -0.41445 ] )
    Troll           = np.polyval( Snuffleupagus, Ernie )
    Hooper          = np.array( [ -0.15212,  0.06849,  0.14950 ] )
...
if __name__ == '__main__' :
    ...
    ans = [  -8.484,  -8.750,  -0.500 ]
    nn  = BirdBathFunc_CD_cls420(  -8.484,  -8.750,   -0.500 )
    print('Fraction of Water = ', nn, '<-- THE BEST ANSWER\n' )


    print('Sensitivity analysis for =/- 0.5 degrees:')
    delta_deg = 1.0
    nn  = BirdBathFunc_CD_cls420(   -8.484+delta_deg,  -8.750,   -0.500 )
    print('Fraction of Water with roll + delta_deg   = ', nn )
    nn  = BirdBathFunc_CD_cls420(   -8.484-delta_deg,  -8.750,   -0.500 )
    print('Fraction of Water with roll - delta_deg   = ', nn )
    nn  = BirdBathFunc_CD_cls420(   -8.484,  -8.750+delta_deg,   -0.500 )
    print('Fraction of Water with tilt + delta_deg   = ', nn )
    nn  = BirdBathFunc_CD_cls420(   -8.484,  -8.750-delta_deg,   -0.500 )
    print('Fraction of Water with tilt - delta_deg   = ', nn )
    nn  = BirdBathFunc_CD_cls420(   -8.484,  -8.750,   -0.500+delta_deg )
        ...
        for twst in np.arange( twst_min, twst_max+deg_inc, deg_inc ) :
            ...
            for tilt in np.arange( tilt_min, tilt_max+deg_inc, deg_inc ) :
                for roll in np.arange( roll_min, roll_max+deg_inc, deg_inc ) :
                    fract = BirdBathFunc_CD_cls420(  roll, tilt, twst )
                    if ( fract >= best_fract ) :
                        best_fract = fract
    ...
    print('\n\n\n')
    nn  = BirdBathFunc_CD_cls420( 8.7500, 0.5000, 8.4844 )
