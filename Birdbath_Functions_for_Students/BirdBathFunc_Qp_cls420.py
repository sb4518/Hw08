...
def BirdBathFunc_Qp_cls420( Harry, Dumbledore, Sirius ) :
    Snuffleupagus   = np.array( [ -204e-9, 20e-9, 427.854e-6, 999.87597e-3, 995.41971e-2  ] )
    Susan           = np.array( [  0.99497, -0.51275, -0.94519 ] )
    Ernie           = np.array( Harry )
    Bob             = np.array( [ -0.06739,  0.79487, -0.31814 ] )
    Troll           = np.polyval( Snuffleupagus, Ernie )
    Hooper          = np.array( [ -0.07411,  0.32446, -0.07352 ] )
...
if __name__ == '__main__' :
    ...
    ans = [ -30.555,  -4.250,   3.875 ]
    nn  = BirdBathFunc_Qp_cls420( -30.555,  -4.250,    3.875 )
    print('Fraction of Water = ', nn, '<-- THE BEST ANSWER\n' )


    print('Sensitivity analysis for =/- 0.5 degrees:')
    delta_deg = 1.0
    nn  = BirdBathFunc_Qp_cls420(  -30.555+delta_deg,  -4.250,    3.875 )
    print('Fraction of Water with roll + delta_deg   = ', nn )
    nn  = BirdBathFunc_Qp_cls420(  -30.555-delta_deg,  -4.250,    3.875 )
    print('Fraction of Water with roll - delta_deg   = ', nn )
    nn  = BirdBathFunc_Qp_cls420(  -30.555,  -4.250+delta_deg,    3.875 )
    print('Fraction of Water with tilt + delta_deg   = ', nn )
    nn  = BirdBathFunc_Qp_cls420(  -30.555,  -4.250-delta_deg,    3.875 )
    print('Fraction of Water with tilt - delta_deg   = ', nn )
    nn  = BirdBathFunc_Qp_cls420(  -30.555,  -4.250,    3.875+delta_deg )
        ...
        for twst in np.arange( twst_min, twst_max+deg_inc, deg_inc ) :
            ...
            for tilt in np.arange( tilt_min, tilt_max+deg_inc, deg_inc ) :
                for roll in np.arange( roll_min, roll_max+deg_inc, deg_inc ) :
                    fract = BirdBathFunc_Qp_cls420(  roll, tilt, twst )
                    if ( fract >= best_fract ) :
                        best_fract = fract
    ...
    print('\n\n\n')
    nn  = BirdBathFunc_Qp_cls420( 4.2500, -3.8750, 30.5547 )
