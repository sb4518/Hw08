...
def BirdBathFunc_CH_cls420( Harry, Dumbledore, Sirius ) :
    Snuffleupagus   = np.array( [ -204e-9, 20e-9, 427.854e-6, 999.87597e-3, 995.41971e-2  ] )
    Susan           = np.array( [  0.98462, -0.61871, -0.87941 ] )
    Ernie           = np.array( Harry )
    Bob             = np.array( [ -0.01934,  0.75769, -0.35755 ] )
    Troll           = np.polyval( Snuffleupagus, Ernie )
    Hooper          = np.array( [  0.17365,  0.20764, -0.31434 ] )
...
if __name__ == '__main__' :
    ...
    ans = [ -33.057,  10.000,   1.125 ]
    nn  = BirdBathFunc_CH_cls420( -33.057,  10.000,    1.125 )
    print('Fraction of Water = ', nn, '<-- THE BEST ANSWER\n' )


    print('Sensitivity analysis for =/- 0.5 degrees:')
    delta_deg = 1.0
    nn  = BirdBathFunc_CH_cls420(  -33.057+delta_deg,  10.000,    1.125 )
    print('Fraction of Water with roll + delta_deg   = ', nn )
    nn  = BirdBathFunc_CH_cls420(  -33.057-delta_deg,  10.000,    1.125 )
    print('Fraction of Water with roll - delta_deg   = ', nn )
    nn  = BirdBathFunc_CH_cls420(  -33.057,  10.000+delta_deg,    1.125 )
    print('Fraction of Water with tilt + delta_deg   = ', nn )
    nn  = BirdBathFunc_CH_cls420(  -33.057,  10.000-delta_deg,    1.125 )
    print('Fraction of Water with tilt - delta_deg   = ', nn )
    nn  = BirdBathFunc_CH_cls420(  -33.057,  10.000,    1.125+delta_deg )
        ...
        for twst in np.arange( twst_min, twst_max+deg_inc, deg_inc ) :
            ...
            for tilt in np.arange( tilt_min, tilt_max+deg_inc, deg_inc ) :
                for roll in np.arange( roll_min, roll_max+deg_inc, deg_inc ) :
                    fract = BirdBathFunc_CH_cls420(  roll, tilt, twst )
                    if ( fract >= best_fract ) :
                        best_fract = fract
    ...
    print('\n\n\n')
    nn  = BirdBathFunc_CH_cls420( -10.0000, -1.1250, 33.0566 )
