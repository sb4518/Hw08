...
def BirdBathFunc_CN_cls420( Harry, Dumbledore, Sirius ) :
    Snuffleupagus   = np.array( [ -204e-9, 20e-9, 427.854e-6, 999.87597e-3, 995.41971e-2  ] )
    Susan           = np.array( [  0.98050, -0.52619, -0.92094 ] )
    Ernie           = np.array( Harry )
    Bob             = np.array( [ -0.04710,  0.83300, -0.36185 ] )
    Troll           = np.polyval( Snuffleupagus, Ernie )
    Hooper          = np.array( [ -0.19081,  0.17101,  0.14472 ] )
...
if __name__ == '__main__' :
    ...
    ans = [ -14.285, -11.000,   2.750 ]
    nn  = BirdBathFunc_CN_cls420( -14.285, -11.000,    2.750 )
    print('Fraction of Water = ', nn, '<-- THE BEST ANSWER\n' )


    print('Sensitivity analysis for =/- 0.5 degrees:')
    delta_deg = 1.0
    nn  = BirdBathFunc_CN_cls420(  -14.285+delta_deg, -11.000,    2.750 )
    print('Fraction of Water with roll + delta_deg   = ', nn )
    nn  = BirdBathFunc_CN_cls420(  -14.285-delta_deg, -11.000,    2.750 )
    print('Fraction of Water with roll - delta_deg   = ', nn )
    nn  = BirdBathFunc_CN_cls420(  -14.285, -11.000+delta_deg,    2.750 )
    print('Fraction of Water with tilt + delta_deg   = ', nn )
    nn  = BirdBathFunc_CN_cls420(  -14.285, -11.000-delta_deg,    2.750 )
    print('Fraction of Water with tilt - delta_deg   = ', nn )
    nn  = BirdBathFunc_CN_cls420(  -14.285, -11.000,    2.750+delta_deg )
        ...
        for twst in np.arange( twst_min, twst_max+deg_inc, deg_inc ) :
            ...
            for tilt in np.arange( tilt_min, tilt_max+deg_inc, deg_inc ) :
                for roll in np.arange( roll_min, roll_max+deg_inc, deg_inc ) :
                    fract = BirdBathFunc_CN_cls420(  roll, tilt, twst )
                    if ( fract >= best_fract ) :
                        best_fract = fract
    ...
    print('\n\n\n')
    nn  = BirdBathFunc_CN_cls420( 11.0000, -2.7500, 14.2852 )
