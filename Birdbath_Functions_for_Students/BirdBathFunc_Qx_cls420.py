...
def BirdBathFunc_Qx_cls420( Harry, Dumbledore, Sirius ) :
    Snuffleupagus   = np.array( [ -204e-9, 20e-9, 427.854e-6, 999.87597e-3, 995.41971e-2  ] )
    Susan           = np.array( [  0.99610, -0.65687, -0.87410 ] )
    Ernie           = np.array( Harry )
    Bob             = np.array( [  0.08277,  0.70582, -0.45493 ] )
    Troll           = np.polyval( Snuffleupagus, Ernie )
    Hooper          = np.array( [  0.03054,  0.26524, -0.17028 ] )
...
if __name__ == '__main__' :
    ...
    ans = [ -30.680,   1.750,  -4.750 ]
    nn  = BirdBathFunc_Qx_cls420( -30.680,   1.750,   -4.750 )
    print('Fraction of Water = ', nn, '<-- THE BEST ANSWER\n' )


    print('Sensitivity analysis for =/- 0.5 degrees:')
    delta_deg = 1.0
    nn  = BirdBathFunc_Qx_cls420(  -30.680+delta_deg,   1.750,   -4.750 )
    print('Fraction of Water with roll + delta_deg   = ', nn )
    nn  = BirdBathFunc_Qx_cls420(  -30.680-delta_deg,   1.750,   -4.750 )
    print('Fraction of Water with roll - delta_deg   = ', nn )
    nn  = BirdBathFunc_Qx_cls420(  -30.680,   1.750+delta_deg,   -4.750 )
    print('Fraction of Water with tilt + delta_deg   = ', nn )
    nn  = BirdBathFunc_Qx_cls420(  -30.680,   1.750-delta_deg,   -4.750 )
    print('Fraction of Water with tilt - delta_deg   = ', nn )
    nn  = BirdBathFunc_Qx_cls420(  -30.680,   1.750,   -4.750+delta_deg )
        ...
        for twst in np.arange( twst_min, twst_max+deg_inc, deg_inc ) :
            ...
            for tilt in np.arange( tilt_min, tilt_max+deg_inc, deg_inc ) :
                for roll in np.arange( roll_min, roll_max+deg_inc, deg_inc ) :
                    fract = BirdBathFunc_Qx_cls420(  roll, tilt, twst )
                    if ( fract >= best_fract ) :
                        best_fract = fract
    ...
    print('\n\n\n')
    nn  = BirdBathFunc_Qx_cls420( -1.7500, 4.7500, 30.6797 )
