...
def BirdBathFunc_CP_cls420( Harry, Dumbledore, Sirius ) :
    Snuffleupagus   = np.array( [ -204e-9, 20e-9, 427.854e-6, 999.87597e-3, 995.41971e-2  ] )
    Susan           = np.array( [  0.96286, -0.65825, -0.83323 ] )
    Ernie           = np.array( Harry )
    Bob             = np.array( [  0.14605,  0.70891, -0.53299 ] )
    Troll           = np.polyval( Snuffleupagus, Ernie )
    Hooper          = np.array( [  0.22708, -0.25333, -0.14719 ] )
...
if __name__ == '__main__' :
    ...
    ans = [  -1.205,  13.125,  -8.625 ]
    nn  = BirdBathFunc_CP_cls420(  -1.205,  13.125,   -8.625 )
    print('Fraction of Water = ', nn, '<-- THE BEST ANSWER\n' )


    print('Sensitivity analysis for =/- 0.5 degrees:')
    delta_deg = 1.0
    nn  = BirdBathFunc_CP_cls420(   -1.205+delta_deg,  13.125,   -8.625 )
    print('Fraction of Water with roll + delta_deg   = ', nn )
    nn  = BirdBathFunc_CP_cls420(   -1.205-delta_deg,  13.125,   -8.625 )
    print('Fraction of Water with roll - delta_deg   = ', nn )
    nn  = BirdBathFunc_CP_cls420(   -1.205,  13.125+delta_deg,   -8.625 )
    print('Fraction of Water with tilt + delta_deg   = ', nn )
    nn  = BirdBathFunc_CP_cls420(   -1.205,  13.125-delta_deg,   -8.625 )
    print('Fraction of Water with tilt - delta_deg   = ', nn )
    nn  = BirdBathFunc_CP_cls420(   -1.205,  13.125,   -8.625+delta_deg )
        ...
        for twst in np.arange( twst_min, twst_max+deg_inc, deg_inc ) :
            ...
            for tilt in np.arange( tilt_min, tilt_max+deg_inc, deg_inc ) :
                for roll in np.arange( roll_min, roll_max+deg_inc, deg_inc ) :
                    fract = BirdBathFunc_CP_cls420(  roll, tilt, twst )
                    if ( fract >= best_fract ) :
                        best_fract = fract
    ...
    print('\n\n\n')
    nn  = BirdBathFunc_CP_cls420( -13.1250, 8.6250, 1.2051 )
