...
def BirdBathFunc_CU_cls420( Harry, Dumbledore, Sirius ) :
    Snuffleupagus   = np.array( [ -204e-9, 20e-9, 427.854e-6, 999.87597e-3, 995.41971e-2  ] )
    Susan           = np.array( [  0.97400, -0.53830, -0.90700 ] )
    Ernie           = np.array( Harry )
    Bob             = np.array( [ -0.05957,  0.84052, -0.35055 ] )
    Troll           = np.polyval( Snuffleupagus, Ernie )
    Hooper          = np.array( [ -0.21857,  0.06138,  0.23340 ] )
...
if __name__ == '__main__' :
    ...
    ans = [  -5.090, -12.625,   3.500 ]
    nn  = BirdBathFunc_CU_cls420(  -5.090, -12.625,    3.500 )
    print('Fraction of Water = ', nn, '<-- THE BEST ANSWER\n' )


    print('Sensitivity analysis for =/- 0.5 degrees:')
    delta_deg = 1.0
    nn  = BirdBathFunc_CU_cls420(   -5.090+delta_deg, -12.625,    3.500 )
    print('Fraction of Water with roll + delta_deg   = ', nn )
    nn  = BirdBathFunc_CU_cls420(   -5.090-delta_deg, -12.625,    3.500 )
    print('Fraction of Water with roll - delta_deg   = ', nn )
    nn  = BirdBathFunc_CU_cls420(   -5.090, -12.625+delta_deg,    3.500 )
    print('Fraction of Water with tilt + delta_deg   = ', nn )
    nn  = BirdBathFunc_CU_cls420(   -5.090, -12.625-delta_deg,    3.500 )
    print('Fraction of Water with tilt - delta_deg   = ', nn )
    nn  = BirdBathFunc_CU_cls420(   -5.090, -12.625,    3.500+delta_deg )
        ...
        for twst in np.arange( twst_min, twst_max+deg_inc, deg_inc ) :
            ...
            for tilt in np.arange( tilt_min, tilt_max+deg_inc, deg_inc ) :
                for roll in np.arange( roll_min, roll_max+deg_inc, deg_inc ) :
                    fract = BirdBathFunc_CU_cls420(  roll, tilt, twst )
                    if ( fract >= best_fract ) :
                        best_fract = fract
    ...
    print('\n\n\n')
    nn  = BirdBathFunc_CU_cls420( 12.6250, -3.5000, 5.0898 )
