...
def BirdBathFunc_CL_cls420( Harry, Dumbledore, Sirius ) :
    Snuffleupagus   = np.array( [ -204e-9, 20e-9, 427.854e-6, 999.87597e-3, 995.41971e-2  ] )
    Susan           = np.array( [  0.97993, -0.72745, -0.81907 ] )
    Ernie           = np.array( Harry )
    Bob             = np.array( [  0.16838,  0.67868, -0.54479 ] )
    Troll           = np.polyval( Snuffleupagus, Ernie )
    Hooper          = np.array( [ -0.10670, -0.10109,  0.17983 ] )
...
if __name__ == '__main__' :
    ...
    ans = [   1.795,  -6.125,  -9.750 ]
    nn  = BirdBathFunc_CL_cls420(   1.795,  -6.125,   -9.750 )
    print('Fraction of Water = ', nn, '<-- THE BEST ANSWER\n' )


    print('Sensitivity analysis for =/- 0.5 degrees:')
    delta_deg = 1.0
    nn  = BirdBathFunc_CL_cls420(    1.795+delta_deg,  -6.125,   -9.750 )
    print('Fraction of Water with roll + delta_deg   = ', nn )
    nn  = BirdBathFunc_CL_cls420(    1.795-delta_deg,  -6.125,   -9.750 )
    print('Fraction of Water with roll - delta_deg   = ', nn )
    nn  = BirdBathFunc_CL_cls420(    1.795,  -6.125+delta_deg,   -9.750 )
    print('Fraction of Water with tilt + delta_deg   = ', nn )
    nn  = BirdBathFunc_CL_cls420(    1.795,  -6.125-delta_deg,   -9.750 )
    print('Fraction of Water with tilt - delta_deg   = ', nn )
    nn  = BirdBathFunc_CL_cls420(    1.795,  -6.125,   -9.750+delta_deg )
        ...
        for twst in np.arange( twst_min, twst_max+deg_inc, deg_inc ) :
            ...
            for tilt in np.arange( tilt_min, tilt_max+deg_inc, deg_inc ) :
                for roll in np.arange( roll_min, roll_max+deg_inc, deg_inc ) :
                    fract = BirdBathFunc_CL_cls420(  roll, tilt, twst )
                    if ( fract >= best_fract ) :
                        best_fract = fract
    ...
    print('\n\n\n')
    nn  = BirdBathFunc_CL_cls420( 6.1250, 9.7500, -1.7949 )
