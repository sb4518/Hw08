...
def BirdBathFunc_Qg_cls420( Harry, Dumbledore, Sirius ) :
    Snuffleupagus   = np.array( [ -204e-9, 20e-9, 427.854e-6, 999.87597e-3, 995.41971e-2  ] )
    Susan           = np.array( [  0.92333, -0.64706, -0.79105 ] )
    Ernie           = np.array( Harry )
    Bob             = np.array( [  0.09297,  0.74545, -0.48719 ] )
    Troll           = np.polyval( Snuffleupagus, Ernie )
    Hooper          = np.array( [  0.37258, -0.16010, -0.36999 ] )
...
if __name__ == '__main__' :
    ...
    ans = [ -14.537,  21.875,  -5.750 ]
    nn  = BirdBathFunc_Qg_cls420( -14.537,  21.875,   -5.750 )
    print('Fraction of Water = ', nn, '<-- THE BEST ANSWER\n' )


    print('Sensitivity analysis for =/- 0.5 degrees:')
    delta_deg = 1.0
    nn  = BirdBathFunc_Qg_cls420(  -14.537+delta_deg,  21.875,   -5.750 )
    print('Fraction of Water with roll + delta_deg   = ', nn )
    nn  = BirdBathFunc_Qg_cls420(  -14.537-delta_deg,  21.875,   -5.750 )
    print('Fraction of Water with roll - delta_deg   = ', nn )
    nn  = BirdBathFunc_Qg_cls420(  -14.537,  21.875+delta_deg,   -5.750 )
    print('Fraction of Water with tilt + delta_deg   = ', nn )
    nn  = BirdBathFunc_Qg_cls420(  -14.537,  21.875-delta_deg,   -5.750 )
    print('Fraction of Water with tilt - delta_deg   = ', nn )
    nn  = BirdBathFunc_Qg_cls420(  -14.537,  21.875,   -5.750+delta_deg )
        ...
        for twst in np.arange( twst_min, twst_max+deg_inc, deg_inc ) :
            ...
            for tilt in np.arange( tilt_min, tilt_max+deg_inc, deg_inc ) :
                for roll in np.arange( roll_min, roll_max+deg_inc, deg_inc ) :
                    fract = BirdBathFunc_Qg_cls420(  roll, tilt, twst )
                    if ( fract >= best_fract ) :
                        best_fract = fract
    ...
    print('\n\n\n')
    nn  = BirdBathFunc_Qg_cls420( -21.8750, 5.7500, 14.5371 )
