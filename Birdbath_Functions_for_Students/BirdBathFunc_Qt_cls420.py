...
def BirdBathFunc_Qt_cls420( Harry, Dumbledore, Sirius ) :
    Snuffleupagus   = np.array( [ -204e-9, 20e-9, 427.854e-6, 999.87597e-3, 995.41971e-2  ] )
    Susan           = np.array( [  0.98151, -0.73167, -0.81886 ] )
    Ernie           = np.array( Harry )
    Bob             = np.array( [  0.19079,  0.67664, -0.57086 ] )
    Troll           = np.polyval( Snuffleupagus, Ernie )
    Hooper          = np.array( [ -0.01527, -0.08260,  0.05999 ] )
...
if __name__ == '__main__' :
    ...
    ans = [  -3.459,  -0.875, -11.000 ]
    nn  = BirdBathFunc_Qt_cls420(  -3.459,  -0.875,  -11.000 )
    print('Fraction of Water = ', nn, '<-- THE BEST ANSWER\n' )


    print('Sensitivity analysis for =/- 0.5 degrees:')
    delta_deg = 1.0
    nn  = BirdBathFunc_Qt_cls420(   -3.459+delta_deg,  -0.875,  -11.000 )
    print('Fraction of Water with roll + delta_deg   = ', nn )
    nn  = BirdBathFunc_Qt_cls420(   -3.459-delta_deg,  -0.875,  -11.000 )
    print('Fraction of Water with roll - delta_deg   = ', nn )
    nn  = BirdBathFunc_Qt_cls420(   -3.459,  -0.875+delta_deg,  -11.000 )
    print('Fraction of Water with tilt + delta_deg   = ', nn )
    nn  = BirdBathFunc_Qt_cls420(   -3.459,  -0.875-delta_deg,  -11.000 )
    print('Fraction of Water with tilt - delta_deg   = ', nn )
    nn  = BirdBathFunc_Qt_cls420(   -3.459,  -0.875,  -11.000+delta_deg )
        ...
        for twst in np.arange( twst_min, twst_max+deg_inc, deg_inc ) :
            ...
            for tilt in np.arange( tilt_min, tilt_max+deg_inc, deg_inc ) :
                for roll in np.arange( roll_min, roll_max+deg_inc, deg_inc ) :
                    fract = BirdBathFunc_Qt_cls420(  roll, tilt, twst )
                    if ( fract >= best_fract ) :
                        best_fract = fract
    ...
    print('\n\n\n')
    nn  = BirdBathFunc_Qt_cls420( 0.8750, 11.0000, 3.4590 )
