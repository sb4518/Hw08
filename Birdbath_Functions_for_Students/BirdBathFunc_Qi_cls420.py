...
def BirdBathFunc_Qi_cls420( Harry, Dumbledore, Sirius ) :
    Snuffleupagus   = np.array( [ -204e-9, 20e-9, 427.854e-6, 999.87597e-3, 995.41971e-2  ] )
    Susan           = np.array( [  0.96904, -0.71064, -0.81436 ] )
    Ernie           = np.array( Harry )
    Bob             = np.array( [  0.15131,  0.70299, -0.53638 ] )
    Troll           = np.polyval( Snuffleupagus, Ernie )
    Hooper          = np.array( [ -0.19509,  0.02829,  0.22165 ] )
...
if __name__ == '__main__' :
    ...
    ans = [  -3.711, -11.250,  -8.875 ]
    nn  = BirdBathFunc_Qi_cls420(  -3.711, -11.250,   -8.875 )
    print('Fraction of Water = ', nn, '<-- THE BEST ANSWER\n' )


    print('Sensitivity analysis for =/- 0.5 degrees:')
    delta_deg = 1.0
    nn  = BirdBathFunc_Qi_cls420(   -3.711+delta_deg, -11.250,   -8.875 )
    print('Fraction of Water with roll + delta_deg   = ', nn )
    nn  = BirdBathFunc_Qi_cls420(   -3.711-delta_deg, -11.250,   -8.875 )
    print('Fraction of Water with roll - delta_deg   = ', nn )
    nn  = BirdBathFunc_Qi_cls420(   -3.711, -11.250+delta_deg,   -8.875 )
    print('Fraction of Water with tilt + delta_deg   = ', nn )
    nn  = BirdBathFunc_Qi_cls420(   -3.711, -11.250-delta_deg,   -8.875 )
    print('Fraction of Water with tilt - delta_deg   = ', nn )
    nn  = BirdBathFunc_Qi_cls420(   -3.711, -11.250,   -8.875+delta_deg )
        ...
        for twst in np.arange( twst_min, twst_max+deg_inc, deg_inc ) :
            ...
            for tilt in np.arange( tilt_min, tilt_max+deg_inc, deg_inc ) :
                for roll in np.arange( roll_min, roll_max+deg_inc, deg_inc ) :
                    fract = BirdBathFunc_Qi_cls420(  roll, tilt, twst )
                    if ( fract >= best_fract ) :
                        best_fract = fract
    ...
    print('\n\n\n')
    nn  = BirdBathFunc_Qi_cls420( 11.2500, 8.8750, 3.7109 )
