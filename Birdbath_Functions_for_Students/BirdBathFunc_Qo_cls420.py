...
def BirdBathFunc_Qo_cls420( Harry, Dumbledore, Sirius ) :
    Snuffleupagus   = np.array( [ -204e-9, 20e-9, 427.854e-6, 999.87597e-3, 995.41971e-2  ] )
    Susan           = np.array( [  0.91870, -0.73938, -0.73904 ] )
    Ernie           = np.array( Harry )
    Bob             = np.array( [  0.22056,  0.65378, -0.59537 ] )
    Troll           = np.polyval( Snuffleupagus, Ernie )
    Hooper          = np.array( [  0.32763, -0.16090, -0.31523 ] )
...
if __name__ == '__main__' :
    ...
    ans = [ -12.391,  19.125, -13.500 ]
    nn  = BirdBathFunc_Qo_cls420( -12.391,  19.125,  -13.500 )
    print('Fraction of Water = ', nn, '<-- THE BEST ANSWER\n' )


    print('Sensitivity analysis for =/- 0.5 degrees:')
    delta_deg = 1.0
    nn  = BirdBathFunc_Qo_cls420(  -12.391+delta_deg,  19.125,  -13.500 )
    print('Fraction of Water with roll + delta_deg   = ', nn )
    nn  = BirdBathFunc_Qo_cls420(  -12.391-delta_deg,  19.125,  -13.500 )
    print('Fraction of Water with roll - delta_deg   = ', nn )
    nn  = BirdBathFunc_Qo_cls420(  -12.391,  19.125+delta_deg,  -13.500 )
    print('Fraction of Water with tilt + delta_deg   = ', nn )
    nn  = BirdBathFunc_Qo_cls420(  -12.391,  19.125-delta_deg,  -13.500 )
    print('Fraction of Water with tilt - delta_deg   = ', nn )
    nn  = BirdBathFunc_Qo_cls420(  -12.391,  19.125,  -13.500+delta_deg )
        ...
        for twst in np.arange( twst_min, twst_max+deg_inc, deg_inc ) :
            ...
            for tilt in np.arange( tilt_min, tilt_max+deg_inc, deg_inc ) :
                for roll in np.arange( roll_min, roll_max+deg_inc, deg_inc ) :
                    fract = BirdBathFunc_Qo_cls420(  roll, tilt, twst )
                    if ( fract >= best_fract ) :
                        best_fract = fract
    ...
    print('\n\n\n')
    nn  = BirdBathFunc_Qo_cls420( -19.1250, 13.5000, 12.3906 )
