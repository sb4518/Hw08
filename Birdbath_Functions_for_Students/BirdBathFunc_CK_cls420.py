...
def BirdBathFunc_CK_cls420( Harry, Dumbledore, Sirius ) :
    Snuffleupagus   = np.array( [ -204e-9, 20e-9, 427.854e-6, 999.87597e-3, 995.41971e-2  ] )
    Susan           = np.array( [  0.99985, -0.59344, -0.91052 ] )
    Ernie           = np.array( Harry )
    Bob             = np.array( [  0.00000,  0.73908, -0.37158 ] )
    Troll           = np.polyval( Snuffleupagus, Ernie )
    Hooper          = np.array( [  0.01745,  0.31875, -0.18135 ] )
...
if __name__ == '__main__' :
    ...
    ans = [ -34.180,   1.000,  -0.000 ]
    nn  = BirdBathFunc_CK_cls420( -34.180,   1.000,   -0.000 )
    print('Fraction of Water = ', nn, '<-- THE BEST ANSWER\n' )


    print('Sensitivity analysis for =/- 0.5 degrees:')
    delta_deg = 1.0
    nn  = BirdBathFunc_CK_cls420(  -34.180+delta_deg,   1.000,   -0.000 )
    print('Fraction of Water with roll + delta_deg   = ', nn )
    nn  = BirdBathFunc_CK_cls420(  -34.180-delta_deg,   1.000,   -0.000 )
    print('Fraction of Water with roll - delta_deg   = ', nn )
    nn  = BirdBathFunc_CK_cls420(  -34.180,   1.000+delta_deg,   -0.000 )
    print('Fraction of Water with tilt + delta_deg   = ', nn )
    nn  = BirdBathFunc_CK_cls420(  -34.180,   1.000-delta_deg,   -0.000 )
    print('Fraction of Water with tilt - delta_deg   = ', nn )
    nn  = BirdBathFunc_CK_cls420(  -34.180,   1.000,   -0.000+delta_deg )
        ...
        for twst in np.arange( twst_min, twst_max+deg_inc, deg_inc ) :
            ...
            for tilt in np.arange( tilt_min, tilt_max+deg_inc, deg_inc ) :
                for roll in np.arange( roll_min, roll_max+deg_inc, deg_inc ) :
                    fract = BirdBathFunc_CK_cls420(  roll, tilt, twst )
                    if ( fract >= best_fract ) :
                        best_fract = fract
    ...
    print('\n\n\n')
    nn  = BirdBathFunc_CK_cls420( -1.0000, 0.0000, 34.1797 )
