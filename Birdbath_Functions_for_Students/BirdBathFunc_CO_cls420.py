...
def BirdBathFunc_CO_cls420( Harry, Dumbledore, Sirius ) :
    Snuffleupagus   = np.array( [ -204e-9, 20e-9, 427.854e-6, 999.87597e-3, 995.41971e-2  ] )
    Susan           = np.array( [  0.92013, -0.54208, -0.83997 ] )
    Ernie           = np.array( Harry )
    Bob             = np.array( [ -0.02610,  0.82237, -0.38189 ] )
    Troll           = np.polyval( Snuffleupagus, Ernie )
    Hooper          = np.array( [  0.39073, -0.17286, -0.38551 ] )
...
if __name__ == '__main__' :
    ...
    ans = [ -14.410,  23.000,   1.625 ]
    nn  = BirdBathFunc_CO_cls420( -14.410,  23.000,    1.625 )
    print('Fraction of Water = ', nn, '<-- THE BEST ANSWER\n' )


    print('Sensitivity analysis for =/- 0.5 degrees:')
    delta_deg = 1.0
    nn  = BirdBathFunc_CO_cls420(  -14.410+delta_deg,  23.000,    1.625 )
    print('Fraction of Water with roll + delta_deg   = ', nn )
    nn  = BirdBathFunc_CO_cls420(  -14.410-delta_deg,  23.000,    1.625 )
    print('Fraction of Water with roll - delta_deg   = ', nn )
    nn  = BirdBathFunc_CO_cls420(  -14.410,  23.000+delta_deg,    1.625 )
    print('Fraction of Water with tilt + delta_deg   = ', nn )
    nn  = BirdBathFunc_CO_cls420(  -14.410,  23.000-delta_deg,    1.625 )
    print('Fraction of Water with tilt - delta_deg   = ', nn )
    nn  = BirdBathFunc_CO_cls420(  -14.410,  23.000,    1.625+delta_deg )
        ...
        for twst in np.arange( twst_min, twst_max+deg_inc, deg_inc ) :
            ...
            for tilt in np.arange( tilt_min, tilt_max+deg_inc, deg_inc ) :
                for roll in np.arange( roll_min, roll_max+deg_inc, deg_inc ) :
                    fract = BirdBathFunc_CO_cls420(  roll, tilt, twst )
                    if ( fract >= best_fract ) :
                        best_fract = fract
    ...
    print('\n\n\n')
    nn  = BirdBathFunc_CO_cls420( -23.0000, -1.6250, 14.4102 )
