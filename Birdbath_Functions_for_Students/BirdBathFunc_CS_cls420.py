...
def BirdBathFunc_CS_cls420( Harry, Dumbledore, Sirius ) :
    Snuffleupagus   = np.array( [ -204e-9, 20e-9, 427.854e-6, 999.87597e-3, 995.41971e-2  ] )
    Susan           = np.array( [  0.98835, -0.70364, -0.84122 ] )
    Ernie           = np.array( Harry )
    Bob             = np.array( [  0.15212,  0.70518, -0.53846 ] )
    Troll           = np.polyval( Snuffleupagus, Ernie )
    Hooper          = np.array( [  0.00436,  0.08726, -0.04915 ] )
...
if __name__ == '__main__' :
    ...
    ans = [ -16.432,   0.250,  -8.750 ]
    nn  = BirdBathFunc_CS_cls420( -16.432,   0.250,   -8.750 )
    print('Fraction of Water = ', nn, '<-- THE BEST ANSWER\n' )


    print('Sensitivity analysis for =/- 0.5 degrees:')
    delta_deg = 1.0
    nn  = BirdBathFunc_CS_cls420(  -16.432+delta_deg,   0.250,   -8.750 )
    print('Fraction of Water with roll + delta_deg   = ', nn )
    nn  = BirdBathFunc_CS_cls420(  -16.432-delta_deg,   0.250,   -8.750 )
    print('Fraction of Water with roll - delta_deg   = ', nn )
    nn  = BirdBathFunc_CS_cls420(  -16.432,   0.250+delta_deg,   -8.750 )
    print('Fraction of Water with tilt + delta_deg   = ', nn )
    nn  = BirdBathFunc_CS_cls420(  -16.432,   0.250-delta_deg,   -8.750 )
    print('Fraction of Water with tilt - delta_deg   = ', nn )
    nn  = BirdBathFunc_CS_cls420(  -16.432,   0.250,   -8.750+delta_deg )
        ...
        for twst in np.arange( twst_min, twst_max+deg_inc, deg_inc ) :
            ...
            for tilt in np.arange( tilt_min, tilt_max+deg_inc, deg_inc ) :
                for roll in np.arange( roll_min, roll_max+deg_inc, deg_inc ) :
                    fract = BirdBathFunc_CS_cls420(  roll, tilt, twst )
                    if ( fract >= best_fract ) :
                        best_fract = fract
    ...
    print('\n\n\n')
    nn  = BirdBathFunc_CS_cls420( -0.2500, 8.7500, 16.4316 )
